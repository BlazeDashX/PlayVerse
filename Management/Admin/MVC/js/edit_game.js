...
imgInput.addEventListener('change', function() {
    ...
    if (file) {
        // Create a fake local URL for the selected file
...
form.addEventListener('submit', function(e) {
    ...
    // Numeric checks
    if(sellEl.value < 0) { alert("Sell price cannot be negative"); isValid = false; }
    if(rentEl.value < 0) { alert("Rent price cannot be negative"); isValid = false; }
    if(stockEl.value < 0) { alert("Stock cannot be negative"); isValid = false; }
